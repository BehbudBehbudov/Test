...
from .models import (ContactUs, slider_news, small_slider, FeaturedNews, NewsletterSubscription, News,
                     PopularNews, CategoryNews, SingleNews, BreakingNews,all_news)
from core.forms import ContactForm, NewsletterForm, CommentForm, UserForm, LoginForm
...
def home_view(request):
    BigSlider = all_news.objects.all().filter(is_active=True,is_big_slider=True).order_by('-created_at')[:3]
